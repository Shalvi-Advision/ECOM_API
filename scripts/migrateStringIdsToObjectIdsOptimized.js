...
// Optimized migration functions with batching
const migrateCategories = async (deptMapping) => {
  ...
  while (hasMore) {
    const categories = await Category.find({}).skip(skip).limit(batchSize);
      ...
      if (deptMapping[category.dept_id]) {
        bulkOps.push({
          updateOne: {
            ...
            update: { $set: { dept_id: deptMapping[category.dept_id] } }
          }
        });
      } else {
    ...
    skip += batchSize;
    console.log(`📊 Processed ${skip} categories, updated ${totalUpdated} so far...`);
  }
...
const migrateSubCategories = async (catMapping) => {
  ...
  while (hasMore) {
    const subCategories = await SubCategory.find({}).skip(skip).limit(batchSize);
      ...
      if (catMapping[subCategory.category_id]) {
        bulkOps.push({
          updateOne: {
            ...
            update: { $set: { category_id: catMapping[subCategory.category_id] } }
          }
        });
      } else {
    ...
    skip += batchSize;
    console.log(`📊 Processed ${skip} subcategories, updated ${totalUpdated} so far...`);
  }
  
  console.log(`✅ Updated ${totalUpdated} subcategories`);
};

const migrateProducts = async (deptMapping, catMapping, subCatMapping) => {
  console.log('🔄 Migrating products...');
  const batchSize = 50; // Smaller batch size for products due to memory constraints
  let skip = 0;
  let totalUpdated = 0;
  let hasMore = true;
...
const runMigration = async () => {
  try {
    console.log('🚀 Starting Optimized String ID to ObjectId migration...\n');
